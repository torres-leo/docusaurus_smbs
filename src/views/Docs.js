...
const Docs = () => {
	const renderArticles = useCallback(
		() =>
			articles.map((article, index) => <Article title={article.title} description={article.description} key={index} />),
		[]
	);
	...
	return (
		<div className='Docs'>
			<Section title='Advanced Tutorials'>
				...
				<div className='Section-articles'>{renderArticles()}</div>

				<p className='Section-text'>
					We will assume that you have finished the guides, and know the basics like how to configure plugins, how to
					write React components, etc. These sections will have plugin authors and code contributors in mind, so we may
					occasionally refer to plugin APIs or other architecture details. Don't panic if you don't understand
					everything😉
				</p>
				...
				<div className='Section-edition'>
					...
					<p className='Section-editedBy'>
						Last updated on <span className='Section-editedBy bold'>Sep 2, 2022</span> by{' '}
